/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   libft.h                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: marvin <marvin@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/25 06:13:42 by marvin            #+#    #+#             */
/*   Updated: 2025/05/25 06:13:42 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef LIBFT_H
# define LIBFT_H

# include <stdlib.h>
# include <unistd.h>
# include <string.h>
# include <stddef.h>

int		ft_atoi(const char *str);
void	ft_bzero(void *s, size_t n);
void	*ft_calloc(size_t nmemb, size_t size);
int		ft_isalnum(int c);
int		ft_isalpha(int c);
int		ft_isascii(int c);
int		ft_isdigit(int c);
int		ft_isprint(int c);
char	*ft_itoa(int n);
void	*ft_memchr(void *s, int c, size_t n);
// Marche pas avec const : *ft_memchr(const void *s, int c, size_t n);
// Erreur, car read too many bytes from s
int		ft_memcmp(void *s1, void *s2, size_t n);
// Marche pas avec const : ft_memcmp(const void *s1, const void *s2, size_t n);
void	*ft_memcpy(void *dest, const void *src, size_t n);
void	*ft_memmove(void *dest, const void *src, size_t n);
void	*ft_memset(void *s, int c, size_t n);
char	**ft_split(char *s, char c);
// Erreur, car /0 pas pris en compte
char	*ft_strchr(char *s, int c);
// Marche pas avec const : *ft_strchr(const char *s, int c);
char	*ft_strdup(const char *s);
char	*ft_strjoin(char *s1, char *s2);
size_t	ft_strlcat(char *dst, const char *src, size_t size);
size_t	ft_strlcpy(char *dst, const char *src, size_t size);
size_t	ft_strlen(const char *str);
char	*ft_strmapi(char *s, char (*f)(unsigned int, char));
//void	*ft_striteri(char *s, char (*f)(unsigned int, char*));
int		ft_strncmp(const char *s1, const char *s2, size_t n);
char	*ft_strnstr(char *s1, char *s2, size_t n);
// Marche pas avec const : *ft_strnstr(const char *s1, const char *s2, size_t n);
// Erreur,car code trop complique
char	*ft_strrchr(char *s, int c);
// Marche pas avec const : *ft_strrchr(const char *s, int c);
char	*ft_strtrim(char *s1, char *set);
char	*ft_substr(char *s, unsigned int start, size_t len);
int		ft_tolower(int c);
int		ft_toupper(int c);


#endif
